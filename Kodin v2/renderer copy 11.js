...
//   this.term.clear();        // clears viewport + resets scroll position
//   this.term.write("\x1b[H"); // move cursor to top-left
//   await printPrompt();       // reprint prompt (cwd info from backend)
// };
...
//       await handleClear();        // print prompt at bottom
//     }
//   });
...
//   // 🔹 Remove old terminal if it exists
//   if (this.term) {
//     try {
//       this.term.dispose();
//     } catch (e) {
...
//   // 🔹 Save output handler in class variable so we can remove it later
...
//   // 🔹 Save resize handler in class variable
//   this._resizeHandler = () => {
//     if (this.fitAddon) {
...
async setupOutput() {
  ...
  // 🔹 Terminal output handler (safe: old listeners removed in preload)
  this._outputHandler = (text) => {
    if (!text) return;
    this.term.write(text.replace(/\r/g, "\r"));
    this.term.scrollToBottom();
...
//     }
//   };
//   this.term.onData(this._inputHandler);   // <-- IMPORTANT
...
case 'sql': {
  ...
  try {
    ...
    if (result.code === 0 && !result.stderr) {
      ...
      this.outputs= (result.stdout || '') + (result.stderr || '')|| "✅ Finished ";
    } else {
