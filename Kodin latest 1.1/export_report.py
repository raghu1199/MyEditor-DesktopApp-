...
def export_report(opened_files, current_user, current_folder, outputs):
    # ✅ Decide save directory: use current_folder if available, else fallback to cwd
    ...
    # ✅ Improved section drawer with wrapping
    def draw_section(title, filepath):
        ...
        max_width = width - 70  # leave margins
        with open(filepath, 'r', encoding='utf-8') as f:
            for line in f:
                wrapped_lines = simpleSplit(line.rstrip(), "Courier", 10, max_width)
    ...
    # ✅ Include question.txt, algorithm.txt, aim.txt (if they exist)
    if current_folder:
