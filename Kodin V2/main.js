...
ipcMain.handle('run-command-stream', async (_, { cmd, args = [] }) => {
  return new Promise((resolve) => {
    let stdout = '';
    let stderr = '';
    ...
    proc.stdout.on('data', (chunk) => {
      ...
      stdout += txt;
      if (mainWindow?.webContents) mainWindow.webContents.send('run-output', { type: 'stdout', text: txt });
    });

    proc.stderr.on('data', (chunk) => {
      const txt = chunk.toString();
      stderr += txt;
      if (mainWindow?.webContents) mainWindow.webContents.send('run-output', { type: 'stderr', text: txt });
    });

    proc.on('error', (err) => resolve({ code: 1, stdout, stderr: stderr + '\n' + err.message }));
    proc.on('close', (code) => resolve({ code, stdout, stderr }));

    const timer = setTimeout(() => { try { proc.kill('SIGTERM'); } catch {} }, 12000);
