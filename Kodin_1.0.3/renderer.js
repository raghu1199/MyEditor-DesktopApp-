  ...
  topbar.innerHTML = `
    <div class="flex items-center justify-between bg-[#1e1e1e] text-gray-200 px-4 h-10 w-full border-b border-[#3c3c3c]">
      <div id="editorActions" class="relative flex space-x-4">
        ...
        <button id="getNotesBtn" class="hover:text-teal-400 hidden">📘 Get Notes</button>
...
//   window.electronAPI.loadCredentials().then(res => {
//     let savedCreds = res.success ? res.data : [];
//     let roleCreds = savedCreds.filter(c => c.role === role);

//     // Prefill institute from cache (take last used for this role)
//     let defaultInstitute = roleCreds.length > 0 ? roleCreds[roleCreds.length - 1].institute : "";
//     console.log("saved cred:",savedCreds,defaultInstitute,roleCreds)
...
//           <h2 class="text-4xl font-extrabold mb-8 text-white text-center">${title}</h2>
//           <form id="loginForm" class="space-y-6">
//             ${this.inputField('Institute', 'text', 'instituteInput', defaultInstitute)}
//             ${isStudent 
//               ? this.inputField('Roll Number', 'text', 'rollInput', roleCreds.length ? roleCreds[roleCreds.length - 1].email_or_roll : "")
//               : this.inputField('Email', 'email', 'emailInput', roleCreds.length ? roleCreds[roleCreds.length - 1].email_or_roll : "")
//             }
//             ${this.inputField('Password', 'password')}
...
//           <!-- Suggestion dropdown for roll/email -->
//           <div id="accountSuggestions" 
//                class="absolute bg-white text-black w-full rounded shadow max-h-48 overflow-y-auto z-50 hidden">
//           </div>
//         </div>
//       </div>
//     `;
...
//       isStudent 
//         ? '#loginForm input[placeholder="Roll Number"]' 
//         : '#loginForm input[placeholder="Email"]'
//     );
//     const suggestionBox = document.getElementById('accountSuggestions');

//     // 🔹 Suggestion list for roll/email only
//     accountInput.addEventListener('focus', () => {
//       if (roleCreds.length > 0) {
//         suggestionBox.innerHTML = roleCreds
//           .map(c => `<div class="px-4 py-2 cursor-pointer hover:bg-gray-200" data-inst="${c.institute}" data-value="${c.email_or_roll}">
//                        ${c.email_or_roll} <span class="text-gray-500 text-sm">(${c.institute})</span>
//                      </div>`)
//           .join('');
...
//       setTimeout(() => suggestionBox.classList.add('hidden'), 200); // delay to allow click
//     });

//     suggestionBox.addEventListener('click', e => {
//       if (e.target.closest('[data-value]')) {
//         const choice = e.target.closest('[data-value]');
//         accountInput.value = choice.dataset.value;
//         instituteInput.value = choice.dataset.inst; // ensure correct institute is set
...
//       const institute = instituteInput.value.trim();
//       const password = document.querySelector('#loginForm input[placeholder="Password"]').value.trim();
//       const email_or_roll = accountInput.value.trim();
...
//           // Save credentials (append if new)
//           window.electronAPI.saveCredentials({ institute, role, email_or_roll });
...
//             <p id="dashboardPoints" class="text-white text-lg">120</p>
//           </div>
//           <div class="bg-[#1e1e1e] p-4 rounded-lg text-center">
//             <h3 class="text-[#61dafb] font-semibold">Lines of Code</h3>
//             <p id="dashboardLOC" class="text-white text-lg">3,450</p>
//           </div>
//           <div class="bg-[#1e1e1e] p-4 rounded-lg text-center">
//             <h3 class="text-[#61dafb] font-semibold">Streak</h3>
//             <p id="dashboardStreak" class="text-white text-lg">7 days</p>
//           </div>
//         </div>
...
//           <div id="leaderboardList" class="space-y-2 max-h-40 overflow-y-auto">
//             <div class="flex justify-between bg-[#1e1e1e] p-2 rounded">
//               <span class="text-white text-sm">#1 Alice</span>
//               <span class="text-[#61dafb] text-sm">220 pts</span>
//             </div>
//             <div class="flex justify-between bg-[#1e1e1e] p-2 rounded">
//               <span class="text-white text-sm">#2 Bob</span>
//               <span class="text-[#61dafb] text-sm">180 pts</span>
//             </div>
//             <div class="flex justify-between bg-[#1e1e1e] p-2 rounded">
//               <span class="text-white text-sm">#3 Charlie</span>
//               <span class="text-[#61dafb] text-sm">150 pts</span>
//             </div>
//           </div>
//         </div>
//       </div>
