...
const { writeShadow, readShadow, scanAndQuarantine } = require("./encryption");
...
ipcMain.handle('dialog:openFolder', async () => {
  ...
  // 🔒 Scan and quarantine files with missing/corrupt shadow
  ...
  return {
    ...
    folderPath,
    tree,
    quarantineRoot, // folder where quarantined files are moved
  };
});
...
// // ------------------------
// // Write Output Temp File with Shadow
// // ------------------------
// ipcMain.handle('write-output-temp', async (_, outputText) => {
//   try {
//     const tempDir = os.tmpdir();
...
//     } catch (err) {
//       if (err.code !== "ENOENT") {
//         console.warn("⚠️ Failed to delete old output file:", err);
//       }
//     }

//     // 1️⃣ Write clean UTF-8 content
//     await fsp.writeFile(fullPath, outputText, { encoding: "utf8" });

//     // 2️⃣ Write shadow mirror
//     await writeShadow(fullPath, outputText);

//     // 3️⃣ Track file for cleanup on quit (avoid duplicates)
//     if (!tempFiles.includes(fullPath)) {
//       tempFiles.push(fullPath);
//     }
...
ipcMain.handle("save-question-files", async (_, { questionText }) => {
  try {
    ...
    // Create each file with shadow
    for (const file of files) {
      const filePath = path.join(fullPath, file.name);

      // 1️⃣ Always create UTF-8 file
...
//     // Skip shadow check for non-Kodin files (like .pdf, .txt outside Kodin sources)
    // if ([".pdf"].includes(ext)) {
...
//       const warning = `// ⚠️ File quarantined. Shadow missing or corrupted.\n// Original moved to ${dest}\n`;
...
//       const warning = `// ⚠️ File altered outside Kodin — opening read-only.\n\n${originalContent}`;
//       return { content: warning, altered: true };
//     }

//     return { content: originalContent };
//   } catch (err) {
//     console.error("Error reading file:", err);
//     return { content: `// ⚠️ Failed to read file: ${err.message}` };
//   }
// });
...
//     if (shadowContent === null) {
//       // Shadow missing → quarantine file
//       const folder = path.dirname(filePath);
//       const quarantineDir = path.join(folder, ".kodin_quarantine");
//       await fsp.mkdir(quarantineDir, { recursive: true });
...
//     if (origHash !== shadowHash) {
//       // File altered outside Kodin → read-only warning
//       const warning = `// ⚠️ File altered outside Kodin — opening read-only.\n\n${originalContent}`;
//       return { content: warning, altered: true };
//     }
...
//   } catch (err) {
//     console.error("Error reading file:", err);
//     // always return string so Monaco never sees null
//     return { content: `// ⚠️ Failed to read file: ${err.message}` };
//   }
// });
