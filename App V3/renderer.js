...
async fetchQuestion(faculty, subject) {
  const institute = this.user.institute || ''; // Make sure it's set
  ...
  try {
    ...
    if (folderPath) {
      ...
      if (!refreshed) {
        ...
        this.currentFolderPath = folderPath; // ✅ Set this
        this.loadFolderToSidebar(refreshed);

      
      alert("Question folder created and loaded.");
    }
  } catch (error) {
...
loadFolderToSidebar(tree) {
  ...
  const renderTree = (items, parent, folderPath) => {
    items.forEach(item => {
      ...
      if (item.type === 'folder') {
        ...
        childrenContainer.className = 'ml-4 space-y-1 hidden'; // Start hidden
        ...
        addFileIcon.onclick = (e) => {
          e.stopPropagation();
          this.createInlineInput(item.path, 'file', childrenContainer);
        };
        ...
        addFolderIcon.onclick = (e) => {
          e.stopPropagation();
          this.createInlineInput(item.path, 'folder', childrenContainer);
        };
  ...
  //rootLabel.innerText = `📁 ${this.currentFolderPath.split(/[\\/]/).pop()}`;
  ...
  rootChildren.className = 'ml-2 space-y-1'; // Show by default
  ...
  if (tree.children) {
  renderTree(tree.children, container, this.currentFolderPath);
}else
{
...
//         addFileIcon.innerText = '＋';
//         addFileIcon.title = 'New File';
//         addFileIcon.className = iconBase;//'cursor-pointer hover:text-white';
//         addFileIcon.onclick = (e) => {
//           e.stopPropagation();
//           this.createInlineInput(item.path, 'file', childrenContainer);
//         };
...
//         addFolderIcon.innerText = '▣';;
//         addFolderIcon.title = 'New Folder';
//         addFolderIcon.className = iconBase //'cursor-pointer hover:text-white';
//         addFolderIcon.onclick = (e) => {
//           e.stopPropagation();
//           this.createInlineInput(item.path, 'folder', childrenContainer);
//         };
...
//   const rootLabel = document.createElement('span');
//   rootLabel.className = 'font-semibold';
//   rootLabel.innerText = `📁 ${this.currentFolderPath.split(/[\\/]/).pop()}`;
...
//   rootAddFile.innerText = '＋';
//   rootAddFile.title = 'New File';
//   rootAddFile.className = iconBase;//'cursor-pointer hover:text-white';
//   rootAddFile.onclick = () => this.createInlineInput(this.currentFolderPath, 'file', container);

//   const rootAddFolder = document.createElement('span');
//   rootAddFolder.innerText = '▣';
//   rootAddFolder.title = 'New Folder';
//   rootAddFolder.className = iconBase; //'cursor-pointer hover:text-white';
//   rootAddFolder.onclick = () => this.createInlineInput(this.currentFolderPath, 'folder', container);
...
//   sidebar.appendChild(rootHeader);
//   if (tree.children) {
//   renderTree(tree.children, container, this.currentFolderPath);
// }
...
//         const actions = document.createElement('div');
//         actions.className = 'space-x-1 flex';

//         const addFileIcon = document.createElement('span');
//         addFileIcon.innerText = '📄';
//         addFileIcon.title = 'New File';
//         addFileIcon.className = 'cursor-pointer text-xs text-gray-400 hover:text-teal-400';
//         addFileIcon.onclick = (e) => {
//           e.stopPropagation();
//           this.createInlineInput(item.path, 'file', childrenContainer);
//         };
...
//         addFolderIcon.innerText = '📁';
//         addFolderIcon.title = 'New Folder';
//         addFolderIcon.className = 'cursor-pointer text-xs text-gray-400 hover:text-teal-400';
//         addFolderIcon.onclick = (e) => {
//           e.stopPropagation();
//           this.createInlineInput(item.path, 'folder', childrenContainer);
//         };
...
//   // Root only shows controls, not name again
//   const rootActions = document.createElement('div');
//   rootActions.className = 'flex justify-between items-center mb-1';

//   const rootLabel = document.createElement('span');
//   rootLabel.className = 'font-semibold text-base';
//   rootLabel.innerText = '📂 ' + this.currentFolderPath.split(/[\\/]/).pop();

//   const rootIcons = document.createElement('div');
//   rootIcons.className = 'space-x-1';

//   const rootAddFile = document.createElement('span');
//   rootAddFile.innerText = '📄';
//   rootAddFile.title = 'New File';
//   rootAddFile.className = 'cursor-pointer text-xs text-gray-400 hover:text-teal-400';
//   rootAddFile.onclick = () => this.createInlineInput(this.currentFolderPath, 'file', container);

//   const rootAddFolder = document.createElement('span');
//   rootAddFolder.innerText = '📁';
//   rootAddFolder.title = 'New Folder';
//   rootAddFolder.className = 'cursor-pointer text-xs text-gray-400 hover:text-teal-400';
//   rootAddFolder.onclick = () => this.createInlineInput(this.currentFolderPath, 'folder', container);

//   rootIcons.appendChild(rootAddFile);
//   rootIcons.appendChild(rootAddFolder);
//   rootActions.appendChild(rootLabel);
//   rootActions.appendChild(rootIcons);

//   sidebar.appendChild(rootActions);
//   renderTree(tree, container, this.currentFolderPath);
//   sidebar.appendChild(container);
// }
