...
async function writeShadow(originalPath, content) {
  ...
  const payload = {
    ...
    data: encryptBuffer(content)
  };

  await fsp.writeFile(shadowPath, JSON.stringify(payload), "utf8");
...
async function readShadow(originalPath) {
  try {
    ...
    const txt = await fsp.readFile(shadowPath, "utf8");
    const meta = JSON.parse(txt);

    // decrypt
    const content = decryptBuffer(Buffer.from(meta.data.data));
...
async function scanAndQuarantine(folderPath, opts = {}) {
  ...
  const quarantineDir = path.join(folderPath, ".quarantine");
  ...
  for (const d of list) {
    ...
    const shadow = await readShadow(abs);

    // ❌ No shadow → quarantine
    if (!shadow) {
      await quarantine(abs, quarantineDir);
      continue;
    }

    // ❌ File modified outside Kodin
    if (stats.mtimeMs > shadow.mtime + 50) {   // 50ms tolerance
      await quarantine(abs, quarantineDir);
      continue;
    }
...
ipcMain.handle("readFile", async (_, filePath) => {
  try {
    ...
    // PDFs: skip shadow system
    if (EXCLUDED_EXTS.includes(ext))  {
      const content = await fsp.readFile(filePath, "utf8");
      return { content };
    }

    const shadow = await readShadow(filePath);

    if (!shadow) {
      // ❌ Missing shadow → quarantine
    ...
    // 1️⃣ mtime check (PRIMARY detector of external edits)
    if (stats.mtimeMs > shadow.mtime + 50) {
      const folder = path.dirname(filePath);
      const quarantineDir = path.join(folder, ".quarantine");
      await fsp.mkdir(quarantineDir, { recursive: true });
    ...
    // 2️⃣ (Optional) hash verification of shadow integrity ONLY
    const verifyShadowHash = hashContent(shadow.content);
    if (verifyShadowHash !== shadow.hash) {
      console.warn("⚠️ Shadow file corrupted — hash mismatch");
      // BUT DO NOT QUARANTINE USER FILE (only shadow is bad)
    }

    // Safe → return original file content
...
//     const shadowHash = shadow.hash;          // ⬅ HASH STORED IN SHADOW
//     const shadowContent = shadow.content;    // ⬅ ACTUAL TEXT RESTORED FROM SHADOW

//     // Fallback safety: ensure correct
//     const verifyShadowHash = hashContent(shadowContent);
//     if (verifyShadowHash !== shadowHash) {
//       console.warn("Shadow corrupted: internal hash mismatch");
//     }

//     // Compare file vs shadow text
//     if (origHash !== verifyShadowHash) {
//       // ❌ Altered outside
//       const folder = path.dirname(filePath);
