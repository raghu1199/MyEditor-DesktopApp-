  ...
  async loadapi() {
    ...
    const initFirebase = async () => {
        try {
            ...
            // console.log("llm and base:",this.base_llm,this.base_server,this.test_server);
...
initFileMenuUserActions() {
    ...
    const roleButtons = [
        ...
        'viewJoinRequests', 'viewMySubmissions', 'logout', 'myClasses','createQuiz','joinQuiz'
    ];
...
async fetchQuestion(faculty, subject, classId) {
  ...
  try {
    ...
    // if (!folderPath) throw new Error("Failed to save question files.");
      ...
      const folderPath = result.folder; // ✅ extract string path
    ...
    inputEl.addEventListener("input", async () => {
        ...
        try {
            ...
            if (this.quizCache.has(key)) {
                ...
                const url = new URL(`${base_server}/get-quizzes`);
...
// OR use native crypto.subtle if available

//  generateCodeHash(editorInstance) {
//   let code = editorInstance.getValue();

//   // 🔹 Normalize: remove all whitespace + newlines
//   let normalized = code.replace(/\s+/g, "");

//   // 🔹 Hash (SHA-256)
//   let hash = CryptoJS.SHA256(normalized).toString();

//   return hash;
// }

// Control flow and logical constructs regex
...
//   (async () => {
//     try {
//       // ✅ Normalize code (ignore spaces & newlines)
//       const normalized = code.replace(/\s+/g, "");
//       const encoder = new TextEncoder();
//       const data = encoder.encode(normalized);

//       // ✅ Compute hash (async, non-blocking)
//       const hashBuffer = await crypto.subtle.digest("SHA-256", data);
//       const hashArray = Array.from(new Uint8Array(hashBuffer));
//       const hash = hashArray.map(b => b.toString(16).padStart(2, "0")).join("");

//       // ✅ Count lines (only non-empty)
...
//   // Initialize submittedFiles set if not already
//   if (!this.submittedFiles) this.submittedFiles = new Set();
...
//   // ✅ Helper function to decide if output is valid for leaderboard
//   const isValidOutput = (output) => {
//     if (!output || !output.trim()) return false; // ignore empty or whitespace
//     const lower = output.toLowerCase();
//     return !(
//       lower.includes("error") ||
//       lower.includes("exception") ||
...
//   // ✅ Helper function to update leaderboard only once per file
//   const handleLeaderboard = () => {
//     if (!this.submittedFiles.has(currentTab.filePath)) {
//       if (isValidOutput(this.outputs)) {
//         this.updateLeaderboard(extension, this.editorInstance.getValue(), this.user);
...
//           this._setInteractiveMode(true, (data) => window.electronAPI.sendInteractiveInput(pid, data));
//           append(' Press Ctrl+Enter to finish program.');
//           let leaderboardUpdated = false; // reset before Ctrl+Enter
//           this.outputs = "";
...
//         handleLeaderboard(); // Only once per file
//         break;
//       }
...
//                 if (!leaderboardUpdated && isValidOutput(result.output)) {
//                   this.updateLeaderboard(extension, this.editorInstance.getValue(), this.user);
//                   leaderboardUpdated = true;
//                 }
//               });
...
//         if (isValidOutput(result.stdout)) {
//           this.updateLeaderboard(extension, this.editorInstance.getValue(), this.user);
//         }
//         return;
//       }
...
//         if (result.code === 0 && isValidOutput(result.stdout)) {
//           append('✔ Executed successfully.');
//           this.updateLeaderboard(extension, this.editorInstance.getValue(), this.user);
//         } else {
//           append('❌ Execution failed.');
...
//   // 🔹 Dispose previous editor instance if any
//   if (this.editorInstance) {
//     this.editorInstance.dispose();
//     this.editorInstance = null;
//   }
...
//   // // 🔹 AHK Detection Counters
//   // let hardwareCount = 0;
//   // let lastHardwareCount = 0;
//   // let lastTextLength = 0;
//   // let lastCheckTime = Date.now();
//   // let violationCount = 0;

//   // // 🔹 Listen for actual hardware keypress counts from iohook
//   // if (window.electronAPI && window.electronAPI.onHardwareKeyCount) {
//   //   window.electronAPI.onHardwareKeyCount((count) => {
//   //     hardwareCount = count;
//   //   });
//   // }
//   // console.log("hardware key count:",hardwareCount);

//   // this.editorInstance.onDidChangeModelContent(() => {
//   // const text = this.editorInstance.getValue();
//   // const textLen = text.length;

//   // const textDelta = textLen - lastTextLength;
//   // const keyDelta = hardwareCount - lastHardwareCount;

//   // // Only check when text increases
//   // if (textDelta > 0) {
//   //   // If characters increased but real hardware key count didn't match
//   //   if (keyDelta < textDelta * 0.7) {
//   //     violationCount++;

//   //     if (violationCount >= 2) {
//   //       this.editorInstance.setValue('');
//   //       this.showToast('Automated typing detected! Please type manually.');
//   //       violationCount = 0;
//   //     } else {
//   //       this.showToast(' Suspicious input detected. Type manually.');
//   //     }
//   //   } else {
//   //     // reset violations slowly if user returns to normal typing
...
//   // === AHK/Automation Detection Counters ===
//   let hardwareCount = 0;
//   let lastHardwareCount = 0;
//   let lastTextLength = 0;
//   let violationCount = 0;
...
//   // Listen for real hardware keypresses from iohook
//   if (window.electronAPI && window.electronAPI.onHardwareKeyCount) {
//     window.electronAPI.onHardwareKeyCount((count) => {
//       hardwareCount = count;
//     });
...
//   // // Listen for baseline sync (after file load)
//   // if (window.electronAPI && window.electronAPI.onSyncTypingBaseline) {
//   //   window.electronAPI.onSyncTypingBaseline((charCount) => {
//   //     hardwareCount = charCount;
//   //     lastHardwareCount = charCount;
//   //     lastTextLength = charCount;
//   //     baselineDone = true;
//   //     console.log(`✅ Baseline synced after file load: ${charCount} chars`);
//   //   });
//   // }

//   // === Monitor typing changes ===
//   this.editorInstance.onDidChangeModelContent(() => {
//     if (!baselineDone) return; // ignore initial load
...
//           this.showToast('Automated typing detected! Please type manually.');
//           violationCount = 0;
//           baselineDone = false; // reset after clearing
//           // re-baseline after clearing
//           setTimeout(() => {
//             const newText = this.editorInstance.getValue();
//             hardwareCount = newText.length;
//             lastHardwareCount = newText.length;
...
//   // === Student restrictions (copy/paste/drag) ===
//   if (this.user?.role === "student") {
//     this.editorInstance.onKeyDown((e) => {
//       const key = e.code.toLowerCase();
//       if (
//         ((e.ctrlKey || e.metaKey) && key === "keyv") ||
//         (e.ctrlKey && e.shiftKey && key === "keyv") ||
//         (e.ctrlKey && e.altKey && key === "keyv") ||
...
//       if (
//         ((e.ctrlKey || e.metaKey) && key === "keyc") ||
//         (e.ctrlKey && e.code === "Insert")
//       ) {
//         e.preventDefault();
//         this.showToast("🚫 Copy is disabled.");
//       }
...
//     this.editorInstance.updateOptions({ contextmenu: false });

//     editorContainer.addEventListener("paste", (e) => { e.preventDefault(); this.showToast("🚫 Paste disabled"); });
//     editorContainer.addEventListener("copy", (e) => { e.preventDefault(); this.showToast("🚫 Copy disabled"); });
//     editorContainer.addEventListener("drop", (e) => { e.preventDefault(); this.showToast("🚫 Drag & drop disabled"); });
