...
// let lastState = false; // tracks whether .ahk scripts were running last scan
...
// /**
//  * Filters out scanner's own PowerShell/cmd/wmic processes
//  */
...
//   return items
//     .map(it => ({
//       pid: Number(it.ProcessId || it.pid || 0),
//       commandLine: String(it.CommandLine || it.commandline || '')
//     }))
//     .filter(i => i.pid && /\.ahk/i.test(i.commandLine))   // keep only real .ahk
//     .filter(i => !ignorePattern.test(i.commandLine));     // remove scanner helpers
// }

// async function listAhkProcesses() {
//   if (process.platform !== 'win32') return [];

//   // PowerShell approach
//   const psCmd = [
//     'Get-CimInstance Win32_Process',
//     "| Where-Object { $_.CommandLine -match '\\.ahk' }",
...
//   } catch (err) {
//     return await listAhkProcessesWMIC();
//   }
// }

// function listAhkProcessesWMIC() {
//   return new Promise(resolve => {
//     exec('wmic process where "CommandLine like \'%.ahk%\'" get ProcessId,CommandLine /FORMAT:CSV', { windowsHide: true }, (err, stdout) => {
//       if (err || !stdout) return resolve([]);
//       const lines = stdout.trim().split(/\r?\n/).filter(Boolean);
//       const results = [];
//       for (const line of lines) {
//         if (/^Node,CommandLine,ProcessId/i.test(line)) continue; // skip header
...
// function startAhkScriptScanner(intervalMs = 3000) {
//   console.log("inside scanner");
//   if (scanInterval) return;

//   // Initial immediate check
//   (async () => {
//     const running = await listAhkProcesses();
//     lastState = running.length > 0;
//     if (lastState) console.log('[AHK-SCANNER] initial scripts running:', running);
//     else console.log('[AHK-SCANNER] initial: no .ahk scripts found');
//   })();
...
//         lastState = isRunning;
//         if (isRunning) console.log('[AHK-SCANNER] .ahk script(s) detected:', running);
//         else console.log('[AHK-SCANNER] .ahk scripts no longer detected');
//       }
//     } catch (e) {
//       console.error('[AHK-SCANNER] scan error', e);
//     }
//   }, intervalMs);
// }

// function stopAhkScriptScanner() {
//   if (scanInterval) {
...
app.whenReady().then(() => {
  ...
  console.log("calling ahk scanner");
...
//           // Match path to PDF in a more forgiving way
...
//               resolve({ success: false, error: "PDF file not found after export" });
//             }
//           } else {
//             console.error("❌ Could not detect PDF path in output");
//             resolve({ success: false, error: "No PDF path found in Python output" });
//           }
//         } else {
//           console.error("❌ Python process exited with error:", errorOutput);
//           resolve({ success: false, error: errorOutput || "Unknown Python error" });
//         }
//       });
//     } catch (err) {
...
// ipcMain.handle("save-question-files", async (_, { questionText }) => {
//   try {
//     // Resolve Desktop path safely
//     const desktopDir = path.join(os.homedir(), "Desktop");
//     const baseDir = path.join(desktopDir, "Kodin", "Questions");

//     // Ensure Desktop exists first (not always guaranteed!)
//     try {
//       await fsp.access(desktopDir);
//     } catch {
...
//     await Promise.all([
//       fsp.writeFile(path.join(fullPath, "question.txt"), questionText || "", "utf-8"),
//       fsp.writeFile(path.join(fullPath, "algorithm.txt"), "", "utf-8"),
//       fsp.writeFile(path.join(fullPath, "aim.txt"), "", "utf-8"),
//       fsp.writeFile(path.join(fullPath, "conclusion.txt"), "", "utf-8"),
//     ]);

//     console.log(" Question folder created:", fullPath);
//     return fullPath;
//   } catch (err) {
//     console.error("❌ Error saving question files:", err.message);
//     return null;
//   }
// });
...
// ipcMain.handle("save-question-files", async (_, { questionText }) => {
//   try {
//     // Desktop path (OneDrive linked or not)
//     const desktopDir = getSafeAndShortPath(path.join(os.homedir(), "Desktop"));
//     const baseDir = getSafeAndShortPath(path.join(desktopDir, "Kodin", "Questions"));
